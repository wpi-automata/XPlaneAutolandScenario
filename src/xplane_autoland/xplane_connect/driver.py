...
class XPlaneDriver:
    def __init__(
        ...
        self._start_elev = (
            start_elev  # (m) beginning elevation for a 3 degree glideslope
        )
        ...
        self._R = np.array(
            [
                [np.cos(self._rotrad), -np.sin(self._rotrad)],
    ...
    def set_orient_pos(self, phi, theta, psi, x, y, h):
        ...
        self._client.sendDREF(
            "sim/flightmodel/position/psi", self._to_local_heading(psi)
        )
