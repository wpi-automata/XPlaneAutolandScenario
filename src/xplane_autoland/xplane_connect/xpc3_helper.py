...
def reset(client, cteInit=0, heInit=0, dtpInit=0, noBrake=True):
    ...
    client.sendPOSI(
        [47.196890, -119.33260, 362.14444, 0.31789625, 0.10021035, 53.7, 1], 0
    )
    # Fine-tune position
    ...
    heading = 53.7 - heInit
    client.sendDREF(
        "sim/flightmodel/position/local_vx", 60.0 * np.sin(heading * np.pi / 180.0)
    )
    client.sendDREF(
        "sim/flightmodel/position/local_vz", -60.0 * np.cos(heading * np.pi / 180.0)
    )

    # Reset fuel levels
    client.sendDREFs(
        ["sim/flightmodel/weight/m_fuel1", "sim/flightmodel/weight/m_fuel2"], [232, 232]
    )
...
def saveState(client, folder, filename="test.csv"):
    ...
    refs = [
        ...
        "Rrad",
        "q",
        "groundspeed" "indicated_airspeed",
        "indicated_airspeed2",
        "M",
        "N",
        "L",
...
def send_ground_velocity(client, v):
    ...
    return client.sendDREFs(
        ["sim/flightmodel/position/local_vx", "sim/flightmodel/position/local_vz"],
        [vx, vz],
    )
...
def dist_from_glideslope(client):
    ...
    pos = np.array([x, y, z])
    return np.linalg.norm(np.cross(pos - HUSMI, pos - UBGUY)) / np.linalg.norm(
        UBGUY - HUSMI
    )
...
def body_frame_velocity(client):
    ...
    Rr = np.array([[1, 0, 0], [0, cos(roll), sin(roll)], [0, -sin(roll), cos(roll)]])
...
def get_autoland_statevec(client):
    ...
    return np.array([vel[0], vel[1], vel[2], P, Q, R, phi, theta, psi, x, y, h]).T
...
def autoland_xy_to_local_xz(client, x, y):
    ...
    R = np.array([[np.cos(rotrad), -np.sin(rotrad)], [np.sin(rotrad), np.cos(rotrad)]])
    ...
    t = np.array([[-25159.26953], [33689.8125]])
