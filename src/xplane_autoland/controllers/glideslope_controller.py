...
class GlideSlopeController:
    def __init__(self, gamma, tch=GRANT_RWY4_TCH, runway_elev=361, des_u=50.0, dt=0.1):
        self._gamma = gamma  # glide slope angle
        self._h_thresh = (
            tch + runway_elev
        )  # height of runway threshold (m) is the TCH + the elevation of the runway (m)
        ...
        if dt > 0.5:
            raise Warning(
                "Running at a much slower dt than controller was designed for"
            )
        ...
        # PI controllers
        # lateral
        self._psi_pid = PID(dt, kp=1.0, ki=0.1, kd=0.0)
        self._y_pid = PID(dt, kp=0.5, ki=0.01, kd=0.0)
        self._phi_pid = PID(dt, kp=1.0, ki=0.1, kd=0.0)

        self._u_pid = PID(dt, kp=50.0, ki=5.0, kd=0.0)
        self._theta_pid = PID(dt, kp=0.24, ki=0.024, kd=0.0)
    ...
    def control(self, statevec, err_h=None):
        ...
        u, v, w, p, q, r, phi, theta, psi, x, y, h = statevec
