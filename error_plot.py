...
def combine_data(save_dir, states, gen_states):
    ...
    with open(
        f"{states}"
    ) as states_file:  # make sure all paths for dataset and generated_states are consistent
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--states-file", help="The states.csv file of dataset", default=None
    )
    parser.add_argument(
        "--gen-states",
        help="The generated_states.csv generated from error_generator.py",
        default=None,
    )
    parser.add_argument(
        "--offset", help="The number of meters in distance offset", default="150"
    )
    ...
    today = date.today()
    save_dir = Path(
        f"{repo_dir}/plots/{today.year}-{today.month}-{today.day}/{args.offset}"
