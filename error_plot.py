...
def plot_data(repo_dir):
    ...
    df = pd.read_csv(f"{repo_dir}/plots/2023-11-27/combined_states.csv", skiprows=lambda x: x%10, usecols=columns)

    plt.scatter(df.error_x, df.x)
    plt.title("Horizontal Distance Error (NN) vs Horizontal Distance (true x)")
    plt.ylabel("Horizontal Distance (True x)")
    plt.xlabel("Horizontal Distance Error (Predicted x)")
    ...
    ax.scatter( df.x, df.psi, df.error_y)
    plt.title("Crosstrack Error (NN) vs Roll (Phi) vs Horizontal Distance")
    ax.set_xlabel("Horizontal Distance (True X)")
